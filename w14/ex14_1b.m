...
for n=1:length(SNR_dB)
 ...
 b_4bits=floor(rand(2,2)*2); % one-bit data
 bx_4bits=(-1).^b_4bits; % {0 1}-->{1 -1}
 x_4bits(1:2,:) = bx_4bits(1 , 1) + j * bx_4bits(1, 2);
 x_4bits(3:4,:) = bx_4bits(2 , 1) + j * bx_4bits(2, 2);
 s_4bits=V*x_4bits;
 ...
 z_4bits=U'*y_4bits;
 sigma=diag(S);
 r_4bits(1) = (z_4bits(1) * sigma(1) +z_4bits(2) * sigma(2))/(sigma(1)^2 + sigma(2)^2);
 r_4bits(2) = (z_4bits(3) * sigma(3) +z_4bits(4) * sigma(4))/(sigma(3)^2 + sigma(4)^2);
 for k =1:2
 b_hat(k,1) = (real(r_4bits(k)) < 0) ;
 b_hat(k,2) = (imag(r_4bits(k)) < 0);
 end
 Count_BER_4bits(m,n)=Count_BER_4bits(m,n)+sum(abs(b_hat(1:2,:)-b_4bits),'all');
 
end
end
end
BER_4bits=Count_BER_4bits./(4*N);
